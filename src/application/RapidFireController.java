...
public class RapidFireController {
	...
	public void setQuestion()
	{
		if(rapidFireCounter<12)
		{
			...
			if(group==0)
				lblQuestionPane.setText(QuestionD.queGrpTopaz[rapidFireCounter][0]);
			else if(group==1)
				lblQuestionPane.setText(QuestionD.queGrpIndicolite[rapidFireCounter][0]);
			else if(group==2)
				lblQuestionPane.setText(QuestionD.queGrpMelopearl[rapidFireCounter][0]);
			else if(group==3)
				lblQuestionPane.setText(QuestionD.queGrpEmerald[rapidFireCounter][0]);
		}
		else
		{
	...
	{
		System.out.println("Show ANswer clicked");
		System.out.println(rapidFireCounter);
		lblQuestions.setWrapText(true);
		lblAnswers.setWrapText(true);
		for(int i=0;i<rapidFireCounter;i++)
		{
			System.out.println("print inside showanswers loop");
			if(group==0)
			{
				lblQuestions.setText(lblQuestions.getText()+QuestionD.queGrpTopaz[i][0]+"\n");
				lblAnswers.setText(lblAnswers.getText()+QuestionD.queGrpTopaz[i][1]+"\n");
			}
			else if(group==1)
			{
				lblQuestions.setText(lblQuestions.getText()+QuestionD.queGrpIndicolite[i][0]+"\n");
				lblAnswers.setText(lblAnswers.getText()+QuestionD.queGrpIndicolite[i][1]+"\n");
			}
			else if(group==2)
			{
				lblQuestions.setText(lblQuestions.getText()+QuestionD.queGrpMelopearl[i][0]+"\n");
				lblAnswers.setText(lblAnswers.getText()+QuestionD.queGrpMelopearl[i][1]+"\n");
			}
			else if(group==3)
			{
				lblQuestions.setText(lblQuestions.getText()+QuestionD.queGrpEmerald[i][0]+"\n");
				lblAnswers.setText(lblAnswers.getText()+QuestionD.queGrpEmerald[i][1]+"\n");
			}
		}
	}
